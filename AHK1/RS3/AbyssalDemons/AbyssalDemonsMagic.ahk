...
teleportSlayerTower() {
	...
	if (ErrorLevel == 0) {
		...
		stringInput := "T" . timingVariable . " P1-1" 
		MouseGetPos, MouseX, MouseY
		RandomBezier(MouseX, MouseY, NormalRand(OutputVarX+5,OutputVarX+20,1), NormalRand(OutputVarY+3,OutputVarY+5,1), stringInput)
		Sleep NormalRand(1041,1214,int=1)
		Send {LButton down}  ; Press down the up-arrow key.
		Sleep NormalRand(23,110,int=1)
		Send {LButton up}
...
checkTurmoil() {
	...
	OutputVarX := 0
	OutputVarY := 0
	ImageSearch, OutputVarX, OutputVarY, X1, Y1, X2, Y2, *50 Torment.png
	if (ErrorLevel == 0) {
