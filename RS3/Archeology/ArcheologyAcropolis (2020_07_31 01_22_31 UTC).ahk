...
Loop{
	...
	clickArea()
	Sleep NormalRand(3000,3000,int=1)
...
clickArcheologyPile() {
	...
	RandomBezier(MouseX, MouseY, NormalRand(724,734,1), NormalRand(441,451,1), stringInput)
	...
	OutputVarX := 0
	OutputVarY := 0
	ImageSearch, OutputVarX, OutputVarY, X1, Y1, X2, Y2, *55 acropolis.png
	If (ErrorLevel == 0){
		timingVariable := NormalRand(748,1048,1)
...
Deposit() {
	...
	stringInput := "T" . timingVariable . " P5-4" 
	MouseGetPos, MouseX, MouseY
	RandomBezier(MouseX, MouseY, NormalRand(1288,1298,1), NormalRand(349,359,1), stringInput)
...
interfaceResetSouth() {
	...
	If (ErrorLevel == 0){
		...
		Sleep NormalRand(3000,3200,int=1)
		Send {Up up}
	} else {
		; MsgBox not found
	}
