...
Loop{
	...
	interfaceReset()
	Sleep NormalRand(30,30,int=1)
	clickArcheologyPileAfterDeposit()
	
	Sleep NormalRand(500,1000,int=1)
...
clickArcheologyPile() {
	...
	RandomBezier(MouseX, MouseY, NormalRand(838,848,1), NormalRand(461,471,1), stringInput)
	...
	OutputVarX := 0
	OutputVarY := 0
	ImageSearch, OutputVarX, OutputVarY, X1, Y1, X2, Y2, *55 goblinRemains.png
	If (ErrorLevel == 0){
		timingVariable := NormalRand(748,1048,1)
...
clickArea() {
	...
	stringInput := "T" . timingVariable . " P5-4" 
	MouseGetPos, MouseX, MouseY
	RandomBezier(MouseX, MouseY, NormalRand(1455,1455,1), NormalRand(143,143,1), stringInput)
...
clickArcheologyPileAfterDeposit() {
	...
	stringInput := "T" . timingVariable . " P5-4" 
	MouseGetPos, MouseX, MouseY
	RandomBezier(MouseX, MouseY, NormalRand(382,392,1), NormalRand(322,332,1), stringInput)
...
Deposit() {
	...
	stringInput := "T" . timingVariable . " P5-4" 
	MouseGetPos, MouseX, MouseY
	RandomBezier(MouseX, MouseY, NormalRand(339,349,1), NormalRand(250,260,1), stringInput)
