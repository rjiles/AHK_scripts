...
Loop{
	...
	Sleep NormalRand(8000,9000,int=1)
	
	Send {Space down}  ; Press down the up-arrow key.
	Sleep NormalRand(50,100,int=1)
	Send {Space up}
	
	interfaceReset()
	
	Sleep NormalRand(110000,115000,int=1)
	...
	Sleep NormalRand(2000,2500,int=1)
	...
	Sleep NormalRand(1000,1500,int=1)
...
clickArcheologyPile() {
	...
	RandomBezier(MouseX, MouseY, NormalRand(806,807,1), NormalRand(506,507,1), stringInput)
...
clickArcheologyPileAfterDeposit() {
	...
	stringInput := "T" . timingVariable . " P5-4" 
	MouseGetPos, MouseX, MouseY
	RandomBezier(MouseX, MouseY, NormalRand(864,865,1), NormalRand(646,647,1), stringInput)
...
Deposit() {
	...
	stringInput := "T" . timingVariable . " P5-4" 
	MouseGetPos, MouseX, MouseY
	RandomBezier(MouseX, MouseY, NormalRand(340,340,1), NormalRand(142,142,1), stringInput)
	...
	OutputVarX := 0
	OutputVarY := 0
	ImageSearch, OutputVarX, OutputVarY, X1, Y1, X2, Y2, *55 bank.png
	If (ErrorLevel == 0){
		timingVariable := NormalRand(748,1048,1)
...
fill() {
	...
	stringInput := "T" . timingVariable . " P5-4" 
	MouseGetPos, MouseX, MouseY
	RandomBezier(MouseX, MouseY, NormalRand(970,970,1), NormalRand(216,216,1), stringInput)
	...
	OutputVarX := 0
	OutputVarY := 0
	ImageSearch, OutputVarX, OutputVarY, X1, Y1, X2, Y2, *55 fill.png
	If (ErrorLevel == 0){
		timingVariable := NormalRand(748,1048,1)
		stringInput := "T" . timingVariable . " P1-1" 
		MouseGetPos, MouseX, MouseY
...
clickSift() {
	...
	stringInput := "T" . timingVariable . " P5-4" 
	MouseGetPos, MouseX, MouseY
	RandomBezier(MouseX, MouseY, NormalRand(322,322,1), NormalRand(186,186,1), stringInput)
	...
	OutputVarX := 0
	OutputVarY := 0
	ImageSearch, OutputVarX, OutputVarY, X1, Y1, X2, Y2, *55 screen.png
	If (ErrorLevel == 0){
