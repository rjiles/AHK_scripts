...
clickSpot1(currentSpot) {
	...
	if (currentSpot == 1) {
		...
		RandomBezier(MouseX, MouseY, NormalRand(946,975,1), NormalRand(640,662,1), stringInput)
		...
		stringInput := "T" . timingVariable . " P9-8" 
		MouseGetPos, MouseX, MouseY
		RandomBezier(MouseX, MouseY, NormalRand(843,875,1), NormalRand(641,665,1), stringInput)
		Sleep NormalRand(250,339,int=1)
		ImageSearch, OutputVarX, OutputVarY, X1, Y1, X2, Y2, *55 fishingSpot.png
		If (ErrorLevel == 0){
			Send {LButton down}  ; Press down the up-arrow key.
...
clickSpot2(currentSpot) {
	...
	if (currentSpot == 1) {
		...
		stringInput := "T" . timingVariable . " P9-8" 
		MouseGetPos, MouseX, MouseY
		RandomBezier(MouseX, MouseY, NormalRand(1032,1079,1), NormalRand(641,672,1), stringInput)
		Sleep NormalRand(250,339,int=1)
		ImageSearch, OutputVarX, OutputVarY, X1, Y1, X2, Y2, *55 fishingSpot.png
		If (ErrorLevel == 0){
			Send {LButton down}  ; Press down the up-arrow key.
		...
		stringInput := "T" . timingVariable . " P9-8" 
		MouseGetPos, MouseX, MouseY
		RandomBezier(MouseX, MouseY, NormalRand(943,981,1), NormalRand(645,668,1), stringInput)
		Sleep NormalRand(250,339,int=1)
		ImageSearch, OutputVarX, OutputVarY, X1, Y1, X2, Y2, *55 fishingSpot.png
		If (ErrorLevel == 0){
			Send {LButton down}  ; Press down the up-arrow key.
